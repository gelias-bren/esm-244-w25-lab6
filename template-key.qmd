...
mom %>%
  ...
  ungroup() |> 
  mutate(prop = n / sum(n)) |> 
  mutate(prop=scales::percent(prop)) |>  # Use for HW
  kable(col.names = c("Risk Level","Number of Women","% of Total")) |> # controls how to design table
  kable_styling() # Makes it look relaitvely clean relatively fast
```
...
rf_final |>
  ...
  fit(risk_level~.,
      data=juice(prep(mom_recipe))) |> # juice 
  vip::vip(geom='point')
